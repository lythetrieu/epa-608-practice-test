...
//   1. Row 1: Section strengths radar | Pacing — 50/50 on sm+, stacked <sm —
//      then the headline stat mini-card row directly under it
//   2. What to practice next — topics + sections + the Weak Spot Drill CTA
//   3. Improvement + Recent tests — side-by-side on sm+, stacked on mobile
//   4. Achievements (collapsed — rank + XP + badge preview, expandable)
...
export function ProgressClient({ userId }: { userId: string }) {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      {/* ── 1. Row 1: Section strengths radar | Pacing — 50/50 on sm+
             (items-stretch + h-full cards for height parity), stacked
             full-width on mobile. The stat mini-card row sits directly
             under it. ─────────────────────────────────────────────────── */}
      {chartData || overview || data.pacing ? (
        <section className="mb-6">
          {!chartData && spots.length > 0 && (
            <p className="text-xs text-steel mb-2">
              Take tests across more sections to unlock your topic proficiency radar.
            </p>
          )}

          {(chartData || data.pacing) && (
            <div
              className={`grid gap-3 items-stretch mb-2 ${
                chartData && data.pacing ? 'sm:grid-cols-2' : ''
              }`}
            >
              {chartData && (
                <div className="bg-white rounded-xl border border-line shadow-card p-4 h-full min-w-0 flex flex-col">
                  <h2 className="font-serif text-lg font-black text-primary-900 mb-2">
                    Section strengths
                  </h2>
              ...
              {/* Pacing — absent/null on stale payloads → radar spans alone */}
              {data.pacing && <PacingSection pacing={data.pacing} />}
            </div>
          )}

          {/* Headline stats — four separate mini cards; absent on stale
              payloads → Row 1 alone. */}
          {overview && (
      ...
      {/* ── 3. Improvement + Recent tests — paired on sm+ (stacked <sm).
      ...
      {/* ── 4. Achievements (LAST) — absent/null on stale payloads → nothing ── */}
