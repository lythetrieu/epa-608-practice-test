...
export function TestClient({ category }: { category: string }) {
  ...
  const handleSubmit = useCallback(async () => {
    if (!sessionId || phase === 'submitted') return
    setPhase('submitted')
    try {
  ...
  if (result) return <ResultView result={result} category={category} />
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b px-6 py-4 flex items-center justify-between">
        <div>
          <span className="text-sm text-gray-500">EPA 608 — {category}</span>
          <div className="text-sm font-semibold text-gray-800 mt-0.5">
            Question {currentIdx + 1} of {questions.length}
          </div>
        </div>
        <div className={`text-lg font-mono font-bold ${timeLeft < 300 ? 'text-red-600' : 'text-gray-700'}`}>
          ⏱ {formatTime(timeLeft)}
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-2xl">
          ...
          <p className="text-xl font-semibold text-gray-900 mb-6 leading-relaxed">{q.question}</p>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-5 py-4 rounded-xl border-2 transition-all flex gap-3 items-start
                    ${selected
                      ? 'border-blue-800 bg-blue-50 text-blue-900'
                      : 'border-gray-200 bg-white hover:border-blue-300 hover:bg-blue-50/50 text-gray-800'
                    }`}
                >
                  <span className={`font-bold text-sm mt-0.5 ${selected ? 'text-blue-800' : 'text-gray-400'}`}>{letter}.</span>
                  <span>{opt}</span>
                </button>
              )
            })}
      ...
      <footer className="bg-white border-t px-6 py-4 flex justify-between items-center">
        <button
          ...
          className="px-5 py-2.5 border border-gray-300 rounded-lg text-gray-700 disabled:opacity-30 hover:bg-gray-50"
        >
          ← Previous
        </button>
        ...
        <div className="flex gap-2 overflow-x-auto max-w-xs">
          {questions.map((_, i) => (
            <button
              ...
              className={`w-7 h-7 rounded text-xs font-medium flex-shrink-0 ${
        ...
        {currentIdx < questions.length - 1 ? (
          <button
            ...
            className="px-5 py-2.5 bg-blue-800 text-white rounded-lg hover:bg-blue-900"
          >
            Next →
          </button>
        ) : (
