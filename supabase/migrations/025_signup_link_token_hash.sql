-- Migration 025: signup confirmation link uses token_hash (no PKCE dead-end)
--
-- Bug: clicking "Confirm" in the signup email landed on /login instead of
-- /dashboard. The signup link went /auth/v1/verify -> /auth/callback?code=,
-- and /auth/callback called exchangeCodeForSession(code). But signup is a raw
-- REST call (/auth/v1/signup), so there is NO PKCE code_verifier in the
-- browser -> the exchange fails -> fall through to /login?error=auth_failed.
--
-- Fix: send the signup link STRAIGHT to /auth/callback with the token_hash
-- (same pattern as recovery). /auth/callback now verifies token_hash
-- server-side via verifyOtp (no code_verifier needed), which confirms the
-- email AND creates the session, then lands the user on /welcome -> dashboard.
--
-- Only the signup branch changes; everything else is identical to 024.
...
BEGIN
  ...
  --   recovery: straight to /reset-password (client verifyOtp).
  --   signup:   straight to /auth/callback with token_hash (server verifyOtp) -> /welcome.
  --   others:   keep the /auth/v1/verify -> /auth/callback mechanism.
  CASE email_action
    WHEN 'recovery' THEN
